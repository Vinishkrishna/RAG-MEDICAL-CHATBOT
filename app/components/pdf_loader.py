...
from langchain.text_splitter import RecursiveCharacterTextSplitter #spliting text to chunks
...
def load_pdf_files(): #It will fetch all pdf files
    try:
        if not os.path.exists(DATA_PATH):
            raise CustomException("Data path doesn't exist")
        logger.info(f"Loading files from {DATA_PATH}")
        loader=DirectoryLoader(DATA_PATH,glob="*.pdf",loader_cls=PyPDFLoader) #It will fetch all the pdf files
